...
HOST_ROOT = Path(os.environ["HERMES_HOST_ROOT"]) if os.environ.get("HERMES_HOST_ROOT") else None
...
def _materialize_host(tmp_path: Path, manifest: CompatibilityManifest) -> Path:
    ...
    sparse_paths = sorted(set(manifest.required_paths) | set(manifest.created_paths) | {
        "pyproject.toml",
        "pytest.ini",
        "tests/conftest.py",
    })
    sparse = _run(["git", "sparse-checkout", "set", "--no-cone", *sparse_paths], cwd=host)
    assert sparse.returncode == 0, sparse.stdout + sparse.stderr
...
def test_real_host_patch_apply_and_memory_authority_contract(tmp_path: Path) -> None:
    if HOST_ROOT is None:
        pytest.skip("set HERMES_HOST_ROOT to a clean Hermes checkout for host-adapter rehearsal")
    if not HOST_ROOT.is_dir():
        pytest.fail(f"Hermes host checkout missing: {HOST_ROOT}")
    ...
    assert manifest.adapter_version == "memory-authority-v1"
    ...
    assert set(manifest.created_paths) == {
        "agent/soullink_memory_authority.py",
